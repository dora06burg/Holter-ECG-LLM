...
def _parse_lead_mask(raw: Any) -> Optional[List[int]]:
    if raw is None:
        return None
    if isinstance(raw, list):
        out: List[int] = []
...
def _normalize_lead_mask(mask: Optional[List[int]], n_channels: int) -> List[int]:
    if not mask:
        return [1] * n_channels
    out = [1 if int(x) != 0 else 0 for x in mask]
    if len(out) < n_channels:
        out = out + [0] * (n_channels - len(out))
    if len(out) > n_channels:
        out = out[:n_channels]
    return out
...
def _write_wfdb_window(
    ...
    *,
    ...
    # fs：采样率（Hz）
    fs: float,
    # signal_mV：窗口内的 ECG 信号（单位 mV，2D 数组，shape=(window_samples, n_channels)）
    ...
    # adc_gain/baseline/units/sig_name：WFDB 头文件所需元信息（这里会扩展成多通道）
    adc_gain: float,
    baseline: int,
    units: str,
    sig_name: str,
    ...
    # lead_order/lead_mask/source_leads：导联元信息（写入 .hea comments）
    lead_order: List[str],
    lead_mask: List[int],
    source_leads: Optional[List[str]],
    ...
    # Waveform
    if signal_mV.ndim == 1:
        signal_mV = signal_mV.reshape(-1, 1)
    n_channels = int(signal_mV.shape[1])
    lead_order_use = list(lead_order) if lead_order else []
    if len(lead_order_use) != n_channels:
        if n_channels == len(LEAD_ORDER):
            lead_order_use = list(LEAD_ORDER)
        else:
            lead_order_use = [f"ch{i}" for i in range(n_channels)]
    lead_mask_use = _normalize_lead_mask(lead_mask, n_channels)
    lead_order_str = ",".join(lead_order_use)
    lead_mask_str = ",".join(str(x) for x in lead_mask_use)
    ...
    comments = [
        ...
        # 采样率
        f"fs={fs}",
        f"lead_order={lead_order_str}",
        f"lead_mask={lead_mask_str}",
        # 数据来源标记
        "source=LSL",
    ]
    if source_leads:
        comments.append(f"source_leads={','.join(str(x) for x in source_leads)}")
    if source_device:
        comments.append(f"source_device={source_device}")
    ...
    sig_name_list = [sig_name] if n_channels == 1 else lead_order_use
    wfdb.wrsamp(
        # record 名（不含扩展名）
        record_name,
        # 采样率
        fs=fs,
        # 每个通道的单位
        units=[units] * n_channels,
        # 每个通道的名字
        sig_name=sig_name_list,
        # 物理信号矩阵：形状 (n_samples, n_channels)
        p_signal=signal_mV.astype(np.float32, copy=False),
        # 存储格式：这里写 16-bit 整型（WFDB 常见格式之一）
        fmt=["16"] * n_channels,
        # adc_gain：物理量到数字量的比例（每通道一个）
        adc_gain=[float(adc_gain)] * n_channels,
        # baseline：基线偏移（每通道一个）
        baseline=[int(baseline)] * n_channels,
        # 写入头文件的 comments
        comments=comments,
        # 输出目录（wfdb 接口需要 str）
...
def main() -> int:
    ...
    default_lead_order = list(LEAD_ORDER) if n_channels == len(LEAD_ORDER) else [f"ch{i}" for i in range(n_channels)]
    current_lead_order: List[str] = list(default_lead_order)
    current_lead_mask: List[int] = [1] * n_channels
    current_source_leads: Optional[List[str]] = None
    current_source_device: Optional[str] = None
    ...
    try:
        ...
        while True:
            ...
            # chunk 非空表示收到了数据
            if chunk:
                # 转成 float32 的 2D numpy 数组 (n_samples, n_channels)
            ...
            if m_chunk:
                ...
                for m_sample in m_chunk:
                    ...
                    if kind == "session_start":
                        ...
                        lead_order = _parse_lead_order(ev.raw.get("lead_order"))
                        lead_mask = _parse_lead_mask(ev.raw.get("lead_mask"))
                        if lead_order:
                            session_meta["lead_order"] = lead_order
                            current_lead_order = list(lead_order)
                        if lead_mask:
                            session_meta["lead_mask"] = lead_mask
                            current_lead_mask = _normalize_lead_mask(lead_mask, n_channels)
                        src_leads = _parse_lead_order(ev.raw.get("source_leads"))
                        if src_leads:
                            session_meta["source_leads"] = src_leads
                            current_source_leads = list(src_leads)
                        src_device = ev.raw.get("source_device")
                        if src_device:
                            session_meta["source_device"] = str(src_device)
                            current_source_device = str(src_device)
                        continue
            ...
            while (
                ...
                _write_wfdb_window(
                    ...
                    lead_order=list(current_lead_order),
                    lead_mask=list(current_lead_mask),
                    source_leads=list(current_source_leads) if current_source_leads else None,
                    source_device=str(current_source_device) if current_source_device else None,
