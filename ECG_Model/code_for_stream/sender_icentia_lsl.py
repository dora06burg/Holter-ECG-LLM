...
#
# 你会得到两条 LSL 流：
# 1) ECG 流（float32，1 通道，名义采样率 = --fs-out）：连续波形样本
...
def _lsl_outlets(pylsl, cfg: StreamConfig, *, fs_out: float, source_id: str, patient_id: int, dataset_root: str):
    ...
    ch = info_ecg.desc().append_child("channels").append_child("channel")  # 添加 channels/channel 节点
    ch.append_child_value("label", "ecg")  # 通道标签：ecg
    ch.append_child_value("unit", "mV")  # 单位：毫伏
...
def main() -> int:
    ...
    for rec in patient_records:
        ...
        n = int(x_out_mV.shape[0])  # 当前 segment 的输出域总样本数
        ...
        while i < n:
            ...
            chunk = x_out_mV[i : i + chunk_samples]  # 取出一个 chunk（一维数组）
            if chunk.size == 0:  # 防御性判断：避免空 chunk
                break
            # pylsl 的 push_chunk 期望形状为 (n_samples, n_channels)，所以 reshape 成二维 (len, 1)
